...
#   ./setup.sh --build      # rebuild from source (skip Docker Hub images)
...
# Docker Hub images (used when not building from source)
IMAGE_KALI="${POWNIE_IMAGE_KALI:-d0gesec/pownie-kali:latest}"
IMAGE_NEO4J="${POWNIE_IMAGE_NEO4J:-d0gesec/pownie-neo4j:latest}"
IMAGE_PLAYWRIGHT="${POWNIE_IMAGE_PLAYWRIGHT:-d0gesec/pownie-playwright:latest}"
...
select_profiles() {
    ...
    # If --build was passed, check for second arg
    if [ "${2:-}" = "--all" ]; then
        ENABLE_BROWSER=true
        ENABLE_TELEMETRY=true
        return
    ...
    if ! $BUILD_FROM_SOURCE; then
        read -rp "$(printf "${CYAN}?${NC} Build from source instead of pulling Docker Hub images? [y/N] ")" ans
        [[ "$ans" =~ ^[Yy] ]] && BUILD_FROM_SOURCE=true
    fi
...
generate_compose() {
    ...
    if $BUILD_FROM_SOURCE; then
      ...
      dockerfile: Dockerfile.neo4j"
        ...
        neo4j_source="    image: ${IMAGE_NEO4J}"
    fi
...
NEO4JEOF
    ...
    if [ "$EXEC_MODE" = "mcp" ] && $ENABLE_BROWSER; then
        local playwright_source
        if $BUILD_FROM_SOURCE; then
            playwright_source="    build:
      context: docker
      dockerfile: Dockerfile.playwright"
        else
            playwright_source="    image: ${IMAGE_PLAYWRIGHT}"
        fi
...
print_summary() {
    ...
    echo "  ./setup.sh --build        rebuild from source"
    echo ""
}
